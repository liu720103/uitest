...
def visiElement(ele):
    try:
        ...
        # print ele[0]
        if ele[0] == "class":
            result = WebDriverWait(driver, 60, 0.5).until(EC.visibility_of_element_located((By.CLASS_NAME, ele[1])))
        if ele[0] == "id":
            result = WebDriverWait(driver, 60, 0.5).until(EC.visibility_of_element_located((By.ID, ele[1])))
        if ele[0] == "xpath":
            result = WebDriverWait(driver, 60, 0.5).until(EC.visibility_of_element_located((By.XPATH, ele[1])))
        return result
    except:
        print('%s页面未找到元素' % ele)
